...
from typing import Set, FrozenSet, Tuple, List, Iterable
...
class ArgsContainer:
    ...
    branch_type: Set[Tuple[str, TBT]] = field(default_factory=set)

    transformer: Set[Tuple[str, PT]] = field(default_factory=set)

    transformer_type: Set[Tuple[str, PTT]] = field(default_factory=set)

    energy_source: Set[Tuple[str, ES]] = field(default_factory=set)

    energy_consumer: Set[Tuple[str, EC]] = field(default_factory=set)

    power_electronics_connection: Set[Tuple[str, PEC]] = field(default_factory=set)
...
class TestBusBranchCreator(BusBranchNetworkCreator[ArgsContainer, TN, TB, TBT, PT, PTT, ES, EC, PEC]):
    ...
    def topological_branch_type_creator(self, bus_branch_network: ArgsContainer, *args) -> Tuple[str, TBT]:
        id_args = (f"tbt_{args[1].mrid}", args)
        bus_branch_network.branch_type.add(id_args)
        return id_args

    def power_transformer_creator(self, bus_branch_network: ArgsContainer, *args) -> Tuple[str, PT]:
    ...
    def power_transformer_type_creator(self, bus_branch_network: ArgsContainer, *args) -> Tuple[str, PTT]:
        id_args = (f"ptt_{args[0].asset_info.mrid}", args)
        bus_branch_network.transformer_type.add(id_args)
        return id_args

    def energy_source_creator(self, bus_branch_network: ArgsContainer, *args) -> Tuple[str, ES]:
        id_args = (f"es_{args[0].mrid}", args)
        bus_branch_network.energy_source.add(id_args)
        return id_args

    def energy_consumer_creator(self, bus_branch_network: ArgsContainer, *args) -> Tuple[str, EC]:
        id_args = (f"ec_{args[0].mrid}", args)
        bus_branch_network.energy_consumer.add(id_args)
        return id_args

    def power_electronics_connection_creator(self, bus_branch_network: ArgsContainer, *args) -> Tuple[str, PEC]:
