...
class NetworkDatabaseReader(BaseDatabaseReader):
    ...
    def _validate_equipment_containers(self):
        ...
        for (className, count) in count_by_class:
            logger.warning(f"{count} {className}s were missing an equipment container.")

        if count_by_class:
            logger.warning(f"A total of {len(missing_containers)} equipment had no associated equipment container. Debug logging will show more details.")

        for equipment in missing_containers:
            logger.debug(f"{equipment} was not assigned to any equipment container.")
    ...
    def _validate_sources(self):
        ...
        def has_been_assigned_to_feeder(energy_source: EnergySource) -> bool:
            return energy_source.is_external_grid \
                ...
                and feeder_start_points.isdisjoint({it.to.mrid for it in connected_equipment(energy_source) if it.to})
