...
class BaseCimReader(ABC):
    ...
    def _load_document(self, document: Document, table: TableDocuments, result_set: ResultSet) -> bool:
        ...
        document.createdDateTime = result_set.get_instant(table.created_date_time.query_index, on_none=None)
        document.authorName = result_set.get_string(table.author_name.query_index, on_none="")
    ...
    def load_name_types(self, table: TableNameTypes, result_set: ResultSet, set_last_name_type: Callable[[str], str]) -> bool:
        ...
        """
        name_type = NameType()
        name_type.name = set_last_name_type(result_set.get_string(table.name_.query_index))
    ...
    def load_names(self, table: TableNames, result_set: ResultSet, set_last_name: Callable[[str], str]) -> bool:
        ...
        self._service.get(result_set.get_string(table.identified_object_mrid.query_index),
                          generate_error=lambda mrid, typ: f"Failed to find {typ} with mRID {mrid} for Name {name_name} [{name_type_name}]").add_name(
            name_type, name_name)
    ...
    def _load_organisation_role(self, organisation_role: OrganisationRole, table: TableOrganisationRoles, result_set: ResultSet) -> bool:
        ...
        :raises SQLException: For any errors encountered reading from the database.
        """
        organisation_role.organisation = self._ensure_get(result_set.get_string(table.organisation_mrid.query_index, None), Organisation)
    ...
    def _load_identified_object(identified_object: IdentifiedObject, table: TableIdentifiedObjects, result_set: ResultSet) -> bool:
        ...
        identified_object.numDiagramObjects = result_set.get_int(table.num_diagram_objects.query_index)
