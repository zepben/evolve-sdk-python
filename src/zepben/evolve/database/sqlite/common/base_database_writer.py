...
class BaseDatabaseWriter(ABC):
    ...
    def __init__(
        ...
        self._database_descriptor: str = f"jdbc:sqlite:{database_file}"
    ...
    def save(self) -> bool:
        ...
        try:
            ...
            self._logger.error(f"Unable to save database: {e}", e)
            status = False
    ...
    def _remove_existing(self) -> bool:
        try:
            ...
            self._logger.error(f"Unable to save database, failed to remove previous instance: {e}", e)
            return False

    def _connect(self) -> bool:
        try:
            self._save_connection = self._get_connection(self._database_descriptor)
            ...
            self._logger.error(f"Failed to connect to the database for saving: {e}", e)
            self._close_connection()
            return False
    ...
    def _prepare_insert_statements(self) -> bool:
        try:
            ...
            self._logger.error(f"Failed to prepare insert statements: {e}", e)
            self._close_connection()
            return False
    ...
    def _create(self) -> bool:
        try:
            ...
            self._logger.error(f"Failed to create database schema: {e}", e)
            return False
    ...
    def _close_connection(self):
        try:
            ...
            self._logger.error(f"Failed to close connection to database: {e}", e)
        ...
        try:
            ...
            with closing(self._save_connection.cursor()) as cur:
                def create_indexes(it: SqliteTable):
                    cur.execute(it.create_indexes_sql)
            ...
            self._logger.error(f"Failed to finalise the database: {e}", e)
            return False
        finally:
            self._close_connection()
