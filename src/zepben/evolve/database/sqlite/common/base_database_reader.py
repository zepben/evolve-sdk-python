...
class BaseDatabaseReader(ABC):
    ...
    def __init__(
        ...
        # todo rename to base_service_reader? Replace with generic type?
        service_reader: BaseServiceReader,
        # todo rename to base_service? Replace with generic type?
        service: BaseService,
    ...
    def _post_load(self) -> bool:
        ...
        for it in self._service.unresolved_references():
            raise ValueError(
                ...
                f"{it.from_ref.mrid} resolving {it.resolver.to_class.__name__} {it.to_mrid}"
            )
